...
const mainModel = {
    ...
    insert: async function (params) {
        return new Promise((resolve, reject) => {
            ...
            if (typeof params.fieldValue !== 'object') return resolve({ success: false, error: 'required data', message: 'missing fields' });
    
            let query = `INSERT INTO ${params.tableName}`;
            const fields = [];
            const placeholder = [];
            const fieldValue = [];
    
            Object.entries(params.fieldValue).forEach(([key, value]) => {
                if (key === "LoginID" && (value === 0 || value === null)) return; // skip if LoginID = 0
                fields.push(key);
                placeholder.push("?");
            ...
            if (fields.length === 0) {
                return resolve({ success: false, error: 'required data', message: 'no fields to insert' });
            }
